...
export function TaskPanel(): JSX.Element | null {
  ...
  return (
    <aside aria-label="Tasks" className="flex h-full w-80 shrink-0 flex-col border-l border-line bg-panel">
      ...
      <div className="flex gap-2 border-b border-line p-3">
        <input className="field min-w-0 flex-1" aria-label="New task" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Task" />
        <button
          className="icon-button"
          aria-label="Add task"
          title="Add task"
