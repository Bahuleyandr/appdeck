  ...
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col border-r border-line bg-panel">
      <div className="flex h-12 items-center justify-between border-b border-line px-3">
        ...
        <button className="icon-button h-7 w-7" title="Add service" onClick={() => setCatalogOpen(true)}>
          <Plus size={16} />
        </button>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {services.length === 0 && (
          <button className="mt-4 w-full rounded-lg border border-dashed border-line px-3 py-6 text-center text-xs text-muted hover:border-accent/60 hover:text-ink" onClick={() => setCatalogOpen(true)}>
        ...
        {services.map((service) => {
          ...
          return (
            ...
            >
              {selected && <span className="absolute bottom-2 left-0 top-2 w-0.5 rounded-full bg-accent" />}
              <button className="flex min-w-0 flex-1 items-center gap-2.5 text-left" onClick={() => void selectService(service.id)}>
                <span
                ...
                <span className={`min-w-0 flex-1 truncate text-sm ${selected ? 'text-ink' : 'text-ink/85'}`}>{service.display_name}</span>
                {count > 0 && (
                  <span className="rounded-full bg-accent px-1.5 py-0.5 text-[11px] font-semibold leading-none text-shell">{count > 99 ? '99+' : count}</span>
                )}
              </button>
              <button
