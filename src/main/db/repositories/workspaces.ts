...
export function listWorkspaces(db: Database.Database, includeDeleted = false): Workspace[] {
  const rows = db
    .prepare(`SELECT * FROM workspaces ${includeDeleted ? '' : 'WHERE deleted_at IS NULL'} ORDER BY position ASC, name ASC`)
    .all() as WorkspaceRow[];
...
export function createWorkspace(
  ...
  const positionRow = db.prepare('SELECT COALESCE(MAX(position), -1) + 1 AS position FROM workspaces').get() as { position: number };
  db.prepare(
    `INSERT INTO workspaces
      (id, name, icon, color, position, focus_rules, sleep_defaults, updated_at, deleted_at, rev, origin_device)
     VALUES (?, ?, ?, ?, ?, '{}', '{"idleMinutes":30}', ?, NULL, 1, ?)`
  ).run(id, input.name, input.icon ?? null, input.color ?? null, positionRow.position, now, deviceId);
  db.prepare(
    `INSERT INTO layouts
...
export function updateWorkspace(
  ...
  deviceId: string,
  id: string,
  patch: Partial<Pick<Workspace, 'name' | 'icon' | 'color' | 'focus_rules' | 'sleep_defaults'>>
): Workspace {
  ...
  db.prepare(
    `UPDATE workspaces
     SET name = ?, icon = ?, color = ?, focus_rules = ?, sleep_defaults = ?,
...
export function tombstoneWorkspace(db: Database.Database, deviceId: string, id: string): void {
  const now = Date.now();
  db.transaction(() => {
    db.prepare('UPDATE workspaces SET deleted_at = ?, updated_at = ?, rev = rev + 1, origin_device = ? WHERE id = ?').run(now, now, deviceId, id);
    db.prepare(
...
export function reorderWorkspaces(db: Database.Database, deviceId: string, orderedIds: string[]): void {
  const now = Date.now();
  const stmt = db.prepare('UPDATE workspaces SET position = ?, updated_at = ?, rev = rev + 1, origin_device = ? WHERE id = ?');
  db.transaction(() => {
