...
export function listServiceInstances(db: Database.Database, workspaceId?: string, includeDeleted = false): ServiceInstance[] {
  if (workspaceId) {
    const rows = db
      .prepare(
        `SELECT si.*
  ...
  const rows = db
    .prepare(`SELECT * FROM service_instances ${includeDeleted ? '' : 'WHERE deleted_at IS NULL'} ORDER BY display_name ASC`)
    .all() as ServiceInstanceRow[];
  return rows.map(mapServiceInstance);
}

export function getServiceInstance(db: Database.Database, id: string): ServiceInstance | null {
  const row = db.prepare('SELECT * FROM service_instances WHERE id = ?').get(id) as ServiceInstanceRow | undefined;
...
export function createServiceInstance(
  ...
  deviceId: string,
  input: { recipeId: string; workspaceId: string; displayName: string; profileId?: string | null; color?: string | null }
): ServiceInstance {
  const id = crypto.randomUUID();
  const now = Date.now();
  const partitionKey = `${SERVICE_PARTITION_PREFIX}${id}`;
  const positionRow = db
    .prepare('SELECT COALESCE(MAX(position), -1) + 1 AS position FROM workspace_services WHERE workspace_id = ?')
    .get(input.workspaceId) as { position: number };
  db.transaction(() => {
    db.prepare(
      `INSERT INTO service_instances
        (id, recipe_id, profile_id, display_name, partition_key, color, pinned, muted, sleep_policy,
         custom_css, custom_js, proxy, user_agent, last_url, updated_at, deleted_at, rev, origin_device)
       VALUES (?, ?, ?, ?, ?, ?, 0, 0, '{}', NULL, NULL, NULL, NULL, NULL, ?, NULL, 1, ?)`
    ).run(id, input.recipeId, input.profileId ?? null, input.displayName, partitionKey, input.color ?? null, now, deviceId);
    db.prepare(
...
export function updateServiceInstance(
  ...
  deviceId: string,
  id: string,
  patch: Partial<Omit<ServiceInstance, 'id' | 'recipe_id' | 'partition_key' | 'updated_at' | 'deleted_at' | 'rev' | 'origin_device'>>
): ServiceInstance {
  ...
  db.prepare(
    `UPDATE service_instances
     SET profile_id = ?, display_name = ?, color = ?, pinned = ?, muted = ?, sleep_policy = ?,
         custom_css = ?, custom_js = ?, proxy = ?, user_agent = ?, last_url = ?,
...
export function tombstoneServiceInstance(db: Database.Database, deviceId: string, id: string): void {
  const now = Date.now();
  db.transaction(() => {
    db.prepare('UPDATE service_instances SET deleted_at = ?, updated_at = ?, rev = rev + 1, origin_device = ? WHERE id = ?').run(
      now,
      now,
      deviceId,
      id
    );
    db.prepare('UPDATE workspace_services SET deleted_at = ?, updated_at = ?, rev = rev + 1, origin_device = ? WHERE service_instance_id = ?').run(
      now,
      now,
      deviceId,
      id
...
export function setServiceLastUrl(db: Database.Database, deviceId: string, id: string, lastUrl: string): void {
  db.prepare('UPDATE service_instances SET last_url = ?, updated_at = ?, rev = rev + 1, origin_device = ? WHERE id = ?').run(
    lastUrl,
    Date.now(),
    deviceId,
    id
