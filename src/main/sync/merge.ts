...
function mergeRecord(db: Database.Database, record: SyncRecord): SyncResult {
  ...
  const incomingWins =
    ...
    (record.updatedAt === existing.updated_at && record.originDevice.localeCompare(existing.origin_device) > 0);
  if (incomingWins) {
    upsertRecord(db, record);
    return { applied: 1, conflicts: 0 };
  }
...
function getExistingMeta(db: Database.Database, record: SyncRecord): ExistingMeta | null {
  ...
  const row = db
    .prepare(`SELECT updated_at, origin_device, rev, deleted_at FROM ${table} WHERE ${idColumn} = ?`)
...
function upsertRecord(db: Database.Database, record: SyncRecord): void {
  switch (record.type) {
      ...
      db.prepare(
        `INSERT INTO workspaces
         ...
         ON CONFLICT(id) DO UPDATE SET
           name = excluded.name, icon = excluded.icon, color = excluded.color, position = excluded.position,
    ...
    case 'profile':
      ...
      ).run(record.data.id, record.data.label, record.data.color, record.updatedAt, record.deletedAt, record.rev, record.originDevice);
      break;
    case 'customRecipe':
      db.prepare(
      ...
      db.prepare(
        `INSERT INTO service_instances
         ...
         ON CONFLICT(id) DO UPDATE SET
           ...
           partition_key = excluded.partition_key, color = excluded.color, pinned = excluded.pinned,
           muted = excluded.muted, sleep_policy = excluded.sleep_policy, custom_css = excluded.custom_css,
...
function purgeOldTombstones(db: Database.Database): void {
  ...
  db.prepare('DELETE FROM custom_recipes WHERE deleted_at IS NOT NULL AND deleted_at < ?').run(cutoff);
  db.prepare('DELETE FROM service_instances WHERE deleted_at IS NOT NULL AND deleted_at < ?').run(cutoff);
  db.prepare('DELETE FROM workspace_services WHERE deleted_at IS NOT NULL AND deleted_at < ?').run(cutoff);
